...
export default function ProductCard({ producto, compact = false }: Props) {
  ...
  // Estado del wishlist derivado de la query cacheada
  ...
  const enWishlist = wishlistItems.some(w => w.productoId === producto.id)

  const toggleMut = useMutation({
    mutationFn: () => toggleWishlist(user!.id, producto.id),
        ...
        {isAuthenticated && (
          <button
            onClick={e => {
              e.preventDefault()
              e.stopPropagation()
              if (!user?.id || toggleMut.isPending) return
              toggleMut.mutate()
            }}
            className={`absolute top-2 right-2 z-20 bg-white/90 rounded-full w-8 h-8 flex items-center justify-center shadow-md transition-all md:opacity-0 md:group-hover:opacity-100 ${enWishlist ? 'md:opacity-100' : ''}`}
            aria-label={enWishlist ? 'Quitar de wishlist' : 'Agregar a wishlist'}
          >
            <svg className={`w-4 h-4 transition-colors ${enWishlist ? 'text-red-500' : 'text-gray-400'}`} fill={enWishlist ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
