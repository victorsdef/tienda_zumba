...
public class CarritoService {
    ...
    public CarritoDTO agregar(Long usuarioId, AgregarItemRequest req) {
        ...
        int cantidadFinal = req.getCantidad() + (existente != null ? existente.getCantidad() : 0);
        int disponible = productoService.getStockDisponible(producto, req.getColor(), req.getTalla());
        if (cantidadFinal > disponible) {
            throw new IllegalStateException("Stock insuficiente para la combinación seleccionada");
        }
