...
public class AdminController {
    ...
    private final OrdenesReportePdfService ordenesReportePdfService;
    // ===================== DASHBOARD =====================
    ...
    @GetMapping("/ordenes/reporte-pdf")
@PreAuthorize("hasAnyRole('ADMIN','VENDEDOR')")
public ResponseEntity<byte[]> descargarReporteOrdenes(
    @RequestParam(required = false) EstadoOrden estado
) {
    byte[] pdf = ordenesReportePdfService.generarReporte(estado);

    String nombre = estado != null
        ? "reporte_pedidos_" + estado.name().toLowerCase() + ".pdf"
        : "reporte_pedidos_todos.pdf";
